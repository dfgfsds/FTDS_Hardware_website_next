...
export default function ProductCard({
  ...
  const router = useRouter();
  const FALLBACK_IMAGE = '/assets/img/placeholder-image.jpg'; // Unoda generic fallback img path
  ...
  return (
    <div className="py-8 md:px-8 w-full">
      <div className={`grid ${gridCols} gap-6`}>
        {products?.map((product: any, i: any) => {
          // Dynamic image source array handle check 
          const productImg = product?.image_urls?.[0] || product?.image || FALLBACK_IMAGE;
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col flex-grow justify-between h-full">
                <div>
                  {/* 
                    FIX: Standard <img> tag avoids strict Next.js width/height validations.
                    Added onError fallback mechanism for broken assets.
                  */}
                  <img
                    src={productImg}
                    alt={product?.name || "Product Image"}
                    className="h-32 object-contain mx-auto mb-2 w-full"
                    loading="lazy"
                    onError={(e) => {
